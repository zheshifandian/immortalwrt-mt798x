...
static int ptm_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    case IFX_PTM_CFG_SET:
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            {
            case 16:
                RX_GAMMA_ITF_CFG(0)->rx_tc_crc_size = 1;
                break;
            case 32:
                RX_GAMMA_ITF_CFG(0)->rx_tc_crc_size = 2;
                break;
            default:
                RX_GAMMA_ITF_CFG(0)->rx_tc_crc_size = 0;
            }
            ...
            if (((IFX_PTM_CFG_T *)ifr->ifr_data)->TxTcCrcGen)
            {
                ...
                {
                case 16:
                    TX_GAMMA_ITF_CFG(0)->tx_tc_crc_size = 1;
                    break;
                case 32:
                    TX_GAMMA_ITF_CFG(0)->tx_tc_crc_size = 2;
                    break;
                default:
