...
define DownloadMethod/svn
	$(call wrap_mirror,$(1),$(2), \
		...
		svn export --non-interactive --trust-server-cert -$(SOURCE_VERSION) $(URL) $(SUBDIR) || \
		svn export --non-interactive -$(SOURCE_VERSION) $(URL) $(SUBDIR) ) && \
		echo "Packing checkout..." && \
		export TAR_TIMESTAMP="`svn info -$(SOURCE_VERSION) --show-item last-changed-date $(URL)`" && \
		$(call dl_tar_pack,$(TMP_DIR)/dl/$(FILE),$(SUBDIR)) && \
		mv $(TMP_DIR)/dl/$(FILE) $(DL_DIR)/ && \
		rm -rf $(SUBDIR); \
	)
...
define DownloadMethod/bzr
	$(call wrap_mirror,$(1),$(2), \
		...
		rm -rf $(SUBDIR) && \
		[ \! -d $(SUBDIR) ] && \
		bzr export --per-file-timestamps -$(SOURCE_VERSION) $(SUBDIR) $(URL) && \
		echo "Packing checkout..." && \
