...
IFX_MEI_DownloadBootPages(DSL_DEV_Device_t *pDev)
{
	...
	for (boot_loop = 1;
		 boot_loop <
		 (DSL_DEV_PRIVATE(pDev)->img_hdr->count);
		 boot_loop++)
	{
...
DSL_BSP_SendCMV(DSL_DEV_Device_t *pDev, u16 *request, int reply, u16 *response) // write cmv to arc, if reply needed, wait for reply
{
	...
	memset(DSL_DEV_PRIVATE(pDev)->CMV_RxMsg, 0,
		   sizeof(DSL_DEV_PRIVATE(pDev)->CMV_RxMsg));
	...
	{
		DSL_DEV_PRIVATE(pDev)->arcmsgav = 0;
		DSL_DEV_PRIVATE(pDev)->reply_count++;
...
IFX_MEI_RunAdslModem(DSL_DEV_Device_t *pDev)
{
	...
	if ((DSL_DEV_PRIVATE(pDev)->img_hdr->count) * sizeof(ARC_SWP_PAGE_HDR) > SDRAM_SEGMENT_SIZE)
	{
				...
				u32 MaxSize, u32 *Buffer, u32 *Dest)
{
	 ...
	 */
	size = (data == GET_DATA) ? (DSL_DEV_PRIVATE(pDev)->img_hdr->page[Page].d_size) : (DSL_DEV_PRIVATE(pDev)->img_hdr->page[Page].p_size);
	 ...
	 */
	i = data ? (DSL_DEV_PRIVATE(pDev)->img_hdr->page[Page].d_offset) : (DSL_DEV_PRIVATE(pDev)->img_hdr->page[Page].p_offset);
	 ...
	 */
	*Dest = data ? (DSL_DEV_PRIVATE(pDev)->img_hdr->page[Page].d_dest) : (DSL_DEV_PRIVATE(pDev)->img_hdr->page[Page].p_dest);
...
int DSL_BSP_ATMLedCBRegister(int (*ifx_adsl_ledcallback)(void))
{
	g_adsl_ledcallback = ifx_adsl_ledcallback;
	return 0;
}

int DSL_BSP_ATMLedCBUnregister(int (*ifx_adsl_ledcallback)(void))
{
		...
		for (idx = 0; idx < i; idx++)
		{
			...
			IFX_MEI_DMSG("bar%d(%X)=%X\n", idx,
						 LTQ_MEI_BASE_ADDR + ME_XMEM_BAR_BASE +
							 idx * 4,
						 (((uint32_t)((ifx_mei_device_private_t *)
										  pDev->pPriv)
							   ->adsl_mem_info[idx]
							   .address) &
						  0x0fffffff));
...
#define AUX_IC_CTRL 0x11
	...
	memcpy((u8 *)(DSL_DEV_PRIVATE(pDev)->adsl_mem_info[0].address + 0x1004),
...
IFX_MEI_InitDevice(int num)
{
	...
	memset(&DSL_DEV_PRIVATE(pDev)->adsl_mem_info[0], 0,
...
int DSL_BSP_DriverHandleDelete(DSL_DEV_Device_t *nHandle)
{
...
int IFX_MEI_Ioctls(DSL_DEV_Device_t *pDev, int from_kernel, unsigned int command, unsigned long lon)
{
	int i = 0;
...
#endif
int DSL_BSP_KernelIoctls(DSL_DEV_Device_t *pDev, unsigned int command,
						 unsigned long lon)
{
	int error = 0;
