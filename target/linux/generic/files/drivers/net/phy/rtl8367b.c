										  ...
										  unsigned txdelay, unsigned rxdelay)
{
	...
	mask = (RTL8367B_EXT_RGMXF_RXDELAY_MASK |
			(RTL8367B_EXT_RGMXF_TXDELAY_MASK << RTL8367B_EXT_RGMXF_TXDELAY_SHIFT));
...
static int rtl8367b_setup(struct rtl8366_smi *smi)
{
	...
	for (i = 0; i < RTL8367B_NUM_PORTS; i++)
		REG_RMW(smi,
				...
				RTL8367B_PORT_MISC_CFG_EGRESS_MODE_MASK << RTL8367B_PORT_MISC_CFG_EGRESS_MODE_SHIFT,
				RTL8367B_PORT_MISC_CFG_EGRESS_MODE_ORIGINAL << RTL8367B_PORT_MISC_CFG_EGRESS_MODE_SHIFT);
								...
								const struct rtl8366_vlan_4k *vlan4k)
{
	...
	data[0] = (vlan4k->member & RTL8367B_TA_VLAN0_MEMBER_MASK) << RTL8367B_TA_VLAN0_MEMBER_SHIFT;
	data[0] |= (vlan4k->untag & RTL8367B_TA_VLAN0_UNTAG_MASK) << RTL8367B_TA_VLAN0_UNTAG_SHIFT;
	data[1] = (vlan4k->fid & RTL8367B_TA_VLAN1_FID_MASK) << RTL8367B_TA_VLAN1_FID_SHIFT;
								...
								const struct rtl8366_vlan_mc *vlanmc)
{
	...
	data[0] = (vlanmc->member & RTL8367B_VLAN_MC0_MEMBER_MASK) << RTL8367B_VLAN_MC0_MEMBER_SHIFT;
	data[1] = (vlanmc->fid & RTL8367B_VLAN_MC1_FID_MASK) << RTL8367B_VLAN_MC1_FID_SHIFT;
	data[2] = 0;
	data[3] = (vlanmc->vid & RTL8367B_VLAN_MC3_EVID_MASK) << RTL8367B_VLAN_MC3_EVID_SHIFT;
...
static int rtl8367b_set_mc_index(struct rtl8366_smi *smi, int port, int index)
{
	...
	return rtl8366_smi_rmwr(smi, RTL8367B_VLAN_PVID_CTRL_REG(port),
							RTL8367B_VLAN_PVID_CTRL_MASK << RTL8367B_VLAN_PVID_CTRL_SHIFT(port),
...
static int rtl8367b_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "cpu_port", &smi->cpu_port) || smi->cpu_port >= smi->num_ports)
